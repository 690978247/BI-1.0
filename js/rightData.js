...
// 打开选择变量弹窗
async function openPop(index, i, prop, type, titleI, id) {
  ...
  //初始化插件
  // $('#datatextblockDevice').selectPage({
  //   showField: 'name',
  //   keyField: 'id',
  //   dropButton: false,
  //   data: `https://apidev.sycdev.com/bi/${appId}/variables/device`,
  //   eAjaxSuccess: function (d) {
  //     let resData = {
  //       totalRow: d.data.pages,
  //       list: d.data.records
  //     }
  //     var result;
  //     if (d) result = resData
      ...
      if (Controls.ControlList[wantindex].ControlType === 'piechart') {
        ...
        Controls.Data.DashBoardChartItemList.forEach((item, i) => {
          if (item.name === Controls.ControlList[wantindex].Name) {
            ...
            //   Controls.Data.DashBoardChartItemList[i].option.Variable.valueAxisArr = tableText
            // } else {
            //   Controls.Data.DashBoardChartItemList[i].option.Variable.valueAxisArr = tableDate
            // }
          }
        })
        ...
        Controls.Data.LineChartItemList.forEach((item, i) => {
          if (item.name === Controls.ControlList[wantindex].Name) {
            // Controls.Data.LineChartItemList[wantindex].variable[i].VariableName = check.name
            // Controls.Data.LineChartItemList[wantindex].option.Variables[i].VariableName = check.name
            // Controls.Data.LineChartItemList[wantindex].option.Variables[i].CheckData = check
            // BarChartData.option.Variables[wantindex].VariableName = check.name
...
function openField(index, i, type, data, chackData) {
  ...
  fields.forEach(d => {
    lis += `<li>
          <input type="checkbox" data-type="${d.dataType}" ${data.FieldName == d.fieldName?'checked':''} name="field-check" value="${d.field}" id="${d.fieldName}" ><label for="${d.fieldName}">${d.fieldName}</label>
      </li>`
  })
  $('#field-ul').append(lis)
}
...
function checkField(event) {
  ...
  if (event.target.checked === true) {
    ...
    event.target.checked = true
    fieldCheck.name = event.target.nextSibling.innerText
    fieldCheck.value = event.target.value
    fieldCheck.type = event.target.dataset.type
  } else {
    event.target.checked = false
    fieldCheck = {}
  }
...
function checkFieldKey(e) {
  ...
  fields.forEach(d => {
    lis += `<li>
          <input type="checkbox" data-type="${d.dataType}" name="field-check" value="${d.field}" id="${d.fieldName}" ><label for="${d.fieldName}">${d.fieldName}</label>
      </li>`
  })
  $('#field-ul').append(lis)
...
function confirmField() {
  ...
  if (Controls.ControlList[wantindex].ControlType === 'barchart') {
    if (!type) { // 判断是第几个表格
      Controls.Data.BarChartItemList.forEach((item, i) => {
        if (item.name === Controls.ControlList[wantindex].Name) {
          // Controls.Data.BarChartItemList[wantindex].variable[i].VariableName = check.name
          // Controls.Data.BarChartItemList[wantindex].option.Variables[i].VariableName = check.name
          // Controls.Data.BarChartItemList[wantindex].option.Variables[i].CheckData = check
          // BarChartData.option.Variables[wantindex].VariableName = check.name
      ...
      Controls.Data.BarChartItemList.forEach((item, i) => {
        if (item.name === Controls.ControlList[wantindex].Name) {
          Controls.Data.BarChartItemList[i].fieldData.FieldName = fieldCheck.name
          Controls.Data.BarChartItemList[i].fieldData.FieldValue = fieldCheck.value
          // Controls.Data.BarChartItemList[i].variable[cindex].FieldValue= fieldCheck.value
          if (fieldCheck.type === 'number') {
          ...
          Controls.Data.BarChartItemList[i].fieldData.CheckData = fieldCheck
    ...
    if (!type) { // 判断是第几个表格
      ...
      Controls.Data.LineChartItemList.forEach((item, i) => {
        if (item.name === Controls.ControlList[wantindex].Name) {
          Controls.Data.LineChartItemList[i].fieldData.FieldName = fieldCheck.name
          Controls.Data.LineChartItemList[i].fieldData.FieldValue = fieldCheck.value
          // Controls.Data.LineChartItemList[i].variable[cindex].FieldValue= fieldCheck.value
          if (fieldCheck.type === 'number') {
            item.fieldData.valueAxisArr = tableNumerical
          } else if (fieldCheck.type === 'text') {
          ...
          Controls.Data.LineChartItemList[i].fieldData.CheckData = fieldCheck
    ...
    changeStr += `<div class="dashboard-area dashboard-start" >
                  ...
                  <div>
                    ...
                    <div class="area-gruops" >
                      ...
                      <img width="50px"  src="${data.BackImg?data.BackImg:'./imgs/defuleUpload.png'}" style="display: ${data.BackSetting === 'img' ? 'block': 'none' }" id="dashboardImg1"/>
...
function changeDatatextblockDevice(e) {
  ...
  if (!$(".select-drop-down").length) {
    ...
    getDatatextblockDeviceList(1).then(res => {
...
//获取关联变量下拉框内容
function getDatatextblockDeviceList(current) {
  let postData = {
    current: current,
    size: 10
  }
