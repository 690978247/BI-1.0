    ...
    for (let i = 0; i < list.length; i++) {
      if (list[i].defaultDataConfig.datatype == '实时数据' || list[i].defaultDataConfig.datatype == '历史数据') {
        ...
        if (list[i].option.Variables) {
          // 折线图  柱状图  饼图
          variable = list[i].option.Variables.filter(x => x.VariableName == '请选择' || x.VariableName == undefined || x.VariableName == '选择')
        }
        ...
        if (variable.length > 0) {
          ...
          appTips.errorMsg(name[0].PropertyList.ComName + '该图表数据不完整，无法进行发布，请填写完整')
          judgeDataType = false
          return
        }
        ...
        if (list[i].name.indexOf("linechart") !== -1 || list[i].name.indexOf("barchart") !== -1) { // 折线图 --- 柱状图业务数据表格判断
          if (list[i].fieldData) {
            if (list[i].fieldData.FieldName == '请选择' || list[i].fieldData.FieldName == '配置字段' || list[i].fieldData.FieldName === undefined) {
              let name = Controls.ControlList.filter(x => x.Name == list[i].name)
              appTips.errorMsg(name[0].PropertyList.ComName + '该图表数据不完整，无法进行发布，请填写完整')
              judgeDataType = false
              return
            ...
            if (list[i].option.Variables) {
              variable = list[i].option.Variables.filter(x => x.FieldName == '配置字段' || x.FieldName == '请选择' || x.FieldName === undefined || x.FieldName == '选择' || x.rangevalue === '')
            }
          ...
          let variable = []
          if (list[i].option.Variable) {
            if (list[i].option.Variable.FieldName === '选择' || list[i].option.Variable.FieldName === '请选择' || list[i].option.Variable.FieldName === undefined || list[i].option.Variable.rangevalue === '') {
              variable.push(list[i].option.Variable)
            }
          ...
          let variable = []
          if (list[i].option.Variables) {
            variable = list[i].option.Variables.filter(x => x.FieldName == '配置字段' || x.FieldName == undefined || x.rangevalue === '')
          }

          if (list[i].fieldData) {
            if (list[i].fieldData.FieldName === '配置字段') {
