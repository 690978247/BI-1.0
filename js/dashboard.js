...
token = token ? token : '96c14f5eb7f045ce92aee7c864b33032'
...
function searchvarible() {
  ...
  let postData = {
    ...
    variableSearchList: [{
      deviceId: searchdata.CheckData.equipmentId,
  ...
  request.post(`/bi/${Controls.User.appId}/variables/all/data`, postData).then(res => {
    ...
    if (res.data.data.length !== 0) {
      ...
      option.series[0].data = res.data.data[searchdata.CheckData.equipmentId + ',' + searchdata.CheckData.name]
      for (let i = 0; i < res.data.data.x.length; i++) {
        ...
        <td  style="text-align:center">${res.data.data[searchdata.CheckData.equipmentId+','+searchdata.CheckData.name][i]}</td>
      </tr>`
      }
    ...
    if (item.ControlType === 'solidrectangle') { //矩形样式
      ...
      html += `<div class="commonModule" draggable="false" data-id="${index}" id="dbl-datatextblock${index}" 
        style="background-color: ${item.PropertyList.BackColor};left: ${item.PropertyList.Left}px;top: ${item.PropertyList.Top}px; width: ${item.PropertyList.Width}px;border-radius: ${item.PropertyList.BorderRadius}px;
          ...
          >
        <span class="moduleShape numShow" id="datatextblock${index}" style="font-family: ${item.PropertyList.FontFamily};font-size: ${item.PropertyList.FontSize}px" >
          ${item.CheckData.name}
        </span>
        </div>`
      ...
      html += `<div class="commonModule" draggable="false" data-id="${index}"
        ...
        text-align:${item.PropertyList.TextAlign};left:0px" value='${item.PropertyList.Text}' onmousedown="inputdown(this,event,${index})" >${item.PropertyList.Text}</textarea>
        </div>
        `
      ...
      html += `<div class="commonModule" draggable="false" data-id="${index}"
        ...
        <textarea id="dynamictext-text${index}" class="moduleShape common-input" contenteditable="true" style="padding:0;background-color: ${item.PropertyList.BackColor};border-radius:${item.PropertyList.BorderRadius}px;border-style: ${item.PropertyList.Style};border-color: ${item.PropertyList.BorderColor}; border-width: ${item.PropertyList.BorderWidth}px;font-weight: ${item.PropertyList.FontWeight};text-decoration: ${item.PropertyList.TextDecoration};
        ...
        onmousedown="inputdown(this,event,${index})">${item.PropertyList.Text}</textarea>
        </div>
        `
      ...
      html += `<div class="commonModule" draggable="false" data-id="${index}"
        ...
        <div class="global-input" onmousedown="toggleItem(event, ${index})" style="height:100%;position:relative">
          ...
          style="left:0px;text-align: ${item.PropertyList.TextAlign};position:absolute;top:0;height:100%;box-shadow: ${item.PropertyList.BoxShadow};color: ${item.PropertyList.Color};border-width: ${item.PropertyList.BorderWidth}px;border-style: ${item.PropertyList.Style};border-color: ${item.PropertyList.BorderColor};border-radius: ${item.PropertyList.BorderRadius}px;text-align: ${item.PropertyList.TextAlign};font-weight: ${item.PropertyList.FontWeight};text-decoration: ${item.PropertyList.TextDecoration}; font-size: ${item.PropertyList.FontSize}px;font-family: ${item.PropertyList.FontFamily};" placeholder="请选择" ></input>
          ...
          <input class="search-input" id="textsearch-input${index}" style="text-align: ${item.PropertyList.TextAlign};box-shadow: ${item.PropertyList.BoxShadow};height:100%;color: ${item.PropertyList.Color};border-width: ${item.PropertyList.BorderWidth}px;border-style: ${item.PropertyList.Style};border-color: ${item.PropertyList.BorderColor};border-radius: ${item.PropertyList.BorderRadius}px;text-align: ${item.PropertyList.TextAlign};font-weight: ${item.PropertyList.FontWeight};text-decoration: ${item.PropertyList.TextDecoration}; font-size: ${item.PropertyList.FontSize}px;font-family: ${item.PropertyList.FontFamily};" placeholder="${item.PropertyList.Placeholder}"
  ...
  for (let i = 0; i < item.DataList.length; i++) {
    if (item.DataList[i].variable !== '选择') {
      if (item.DataList[i].CheckData) {
        await request.get(`/bi/${appId}/variables/${item.DataList[i].CheckData.equipmentId}/${item.DataList[i].CheckData.id}/${item.DataList[i].CheckData.name}/status`).then(res => {
      ...
      if (c.defaultDataConfig.datatype === '业务数据') { //业务数据
        ...
        Controls.ControlList.forEach(item => {
          if (item.ControlType === 'associatedatetimepicker') {
            ...
            // let types = ['month', 'year'] // 年、月结束时间不需要加一天
            // let value = ''
          ...
          request.post(`/bi/${appId}/variables/variable-code/history`, postData).then(res => {
            ...
            if (document.getElementById(`${c.name}`)) {
              ...
              res.data.data.x.forEach((kk, i) => {
                c.option.Variables.forEach(d => {
                  let name = d.CheckData.equipmentId + ',' + d.CheckData.name
                  if (name == kk) {
    ...
    if (c.name === item.Name) {
      if (c.defaultDataConfig.datatype === '实时数据') {
        ...
        if (Controls.Data.DashBoardChartItemList[cIndex].option.Variable.CheckData) {
          postData = [{
            deviceId: Controls.Data.DashBoardChartItemList[cIndex].option.Variable.CheckData.equipmentId,
...
function dblOpen(index, i, prop) {
  ...
  let postData = {
    ...
    startTime: new Date(varstarttime).getTime(),
    variableSearchList: [{
      deviceId: index.CheckData.equipmentId,
  ...
  request.post(`/bi/${Controls.User.appId}/variables/all/data`, postData).then(res => {
    ...
    if (res.data.data.length !== 0) {
      ...
      for (let i = 0; i < res.data.data.x.length; i++) {
        ...
        <td style="padding-left:10px" class="table-checkbox">${i+1}</td>
        <td  style="text-align:center">${res.data.data.x[i]}</td>
        <td  style="text-align:center">${res.data.data[index.CheckData.equipmentId+','+index.CheckData.name][i]}</td>
      </tr>`
      }
